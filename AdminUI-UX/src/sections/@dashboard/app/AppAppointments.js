...
const columns = [
  ...
  { id: "doctorId", label: "Doctor ID", minWidth: 170, align: "left" },
...
export default function AppAppointments({ title, subheader, list, ...other }) {
  ...
  useEffect(() => {
    const getAppointmentList = async () => {
      setIsLoading(true);
      try {
        const response = await appointmentService.getAllAppointment(1, 20);
